...
Spotify.prototype.sendPong = function(ping) {
  console.log('Spotify.prototype.sendPong ping: %s', ping)
  this.agent
  .get('http://spotlite.cloudapp.net/api/pong/' + ping.split(" ").join("_"))
  .set({ 'User-Agent': this.userAgent })
  .type("json")
  .end(function (err, res) {
    if (err) return this.emit('error', err);
    ...
    //if (res.body.status == 100) {
      pong = res.body;
      console.log('Spotify.prototype.sendPong pong: %s', pong);
      this.sendCommand('sp/pong_flash2', [ pong ]);
    //}
  }.bind(this));
};
